...
## dummies for kids/adults
formula.food1 <- formula(inadeq.dsum ~ lny + k1 + k2 + k3 + k4 + 
...
## smooth adults and kids
formula.food2 <- formula(inadeq.sum ~ lny + kids + adults  + age + hhh.male + 
                           ethnic + province + settle + marital + education )

food.ol2 <- polr(formula.food2,
                      data=base.data,
                      Hess = TRUE)
...
food.ol2.coef <- coefficients(summary(food.ol2))
bcut <- food.ol2.coef["0|1",1]
by <- food.ol2.coef["lny",1]
bk <- food.ol2.coef["kids",1]
ba <- food.ol2.coef["adults",1]
...
zed <- akmat[,1:2] %>%
  mutate(adults = as.numeric(ao),
         ...
         rhs = (1/by) * (bcut - ba * adults - bk * kids),
         # then exponentiate
