...
house.df <- household %>%
  dplyr::select(
    ...
    Q6105IMAGINE, Q116PRESENT, hholds_wgt, Q221AFOOD) %>%
  filter(Q6105IMAGINE != 9, Q116PRESENT != 9, Q221AFOOD != 9) %>% ## loses 493 obs..
  transmute(
...
```{r loess-plot, dependson="lcs; np-plot-lyn-lnminq", fig.cap = "Nonparametric regression plot and data points: log reported minimum income against log reported income.",}
...
Here, we predict required minimum income, based on the intersection rule. We also use all model components, so that poverty is completely "local"?
...
```{r lineary-poor, dependson="lcs;lineary-spl"}

# This builds out a simple table...
# How do we want to use this?
poor.data <- left_join(main.df, spl.linminq.yall,
                       by = c("ao","ko")) %>%
  mutate(hhsize = as.numeric(ao) + as.numeric(ko),
         ...
         x = exp(lnx)) %>%
  group_by(ao,ko) %>%
  summarise(subj.poor = sum(y < spl)/n(),
            obj.poor.lb = sum(y < lbpoverty)/n(),
            obj.poor.ub = sum(y < ubpoverty)/n(),
            meany = mean(y)) %>%
  ungroup()
...
## Food Models and Predictions
formula.food1 <- formula(adequate~ lnx + k1 + k2 + k3 + k4 + 
                           a2 + a3 + a4 + a5 + a6  + age + hhh.male + 
                           ethnic + province + settle + marital + education + food.share)

food.ol1 <- polr(formula.food1,
                      data=base.data,
                      Hess = TRUE)

cm.food.ol1 <- table(base.data$adequate,
                      predict(food.ol1, base.data, type="class"))

food.ol.sum1 <- summary(food.ol1)$coefficients[,1:2]

## Clothing Models and Predictions
formula.clothing1 <- formula(clothing~ lnx + k1 + k2 + k3 + k4 +  
                             a2 + a3 + a4 + a5 + a6 + age + hhh.male + 
                             ethnic + province + settle + marital + education + clothing.share)
...
```{r order-eq, dependson="lcs2;order-exog"}
...
# Not sure I want to use this, but I would like to keep them similar for all
# However, with a linear model, it really should not matter
#    hhh.male = uocquantile(base.data$hhh.male,0.5), # female
#    ethnic = uocquantile(base.data$ethnic,0.5), # african
#    settle = uocquantile(base.data$settle,0.5), # traditional area
#    marital = uocquantile(base.data$marital,0.5), # never married
#    education = uocquantile(base.data$education,0.5), # nqf0 or primary (i think)
#    age = uocquantile(base.data$age, 0.5),  #48
#    age.sq = uocquantile(base.data$age.sq, 0.5), # 2304
...
## clothing
b <- coef(clothing.ol1)[1:10] # does not count non-income and non-adult/child vars.
b1 <- -b[1]
E.base2 <-  exp( (dfdiff %*% b[-1]) * (1/b1) )

## clothing
b <- coef(housing.ol1)[1:10] # does not count non-income and non-adult/child vars.
b1 <- -b[1]
E.base3 <-  exp( (dfdiff %*% b[-1]) * (1/b1) )
...
for (i in 1:399){
  ...
  ## clothing
  clothing <- polr(formula.clothing1,
                  data=dd,
                  Hess = TRUE)
  cc <- coef(clothing)[1:10]
  cc1 <- - cc[1]
  es.boot <-  exp( (dfdiff %*% cc[-1]) * (1/cc1) )
  E.boot2 <- cbind(E.boot2,es.boot)
  ## housing
  housing <- polr(formula.housing1,
                  data=dd,
                  Hess = TRUE)
  cc <- coef(housing)[1:10]
  cc1 <- - cc[1]
  es.boot <-  exp( (dfdiff %*% cc[-1]) * (1/cc1) )
  E.boot3 <- cbind(E.boot3,es.boot)
}

## Get Eq Scale Standard Deviations
E.sd1 <- apply(E.boot1,1,sd)
E.sd2 <- apply(E.boot2,1,sd)
E.sd3 <- apply(E.boot3,1,sd)
...
# Subjective poverty estimates from linear parametric models

## Categorical adult and child variables
...
## Continuous adult and child variables
