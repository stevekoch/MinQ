...
```{r np-spl-bw1, dependson="lcs-np-data; lcs-sub"}

# set the data to all, which is vectorised for np.
d1 <- df %>%
  dplyr::select(lnx, lny, adults, kids, hhh.male, marital, ethnic, province, 
                settle, education, lnminq, adults1, adults2, kids1, kids2)
...
for (j in 1:dim(akmat)[1]){
   ...
   suppressWarnings(
    ...
    NP.spl1 <- rbind(NP.spl1,spl.val)
}

# just for looking at results?
rownames(NP.spl1) <- NULL
