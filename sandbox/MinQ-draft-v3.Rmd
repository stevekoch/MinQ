...
zed <- akmat[,1:2] %>%
  mutate(adults = as.numeric(ao),
         ...
         ymin.fc = exp(rhsfc),
         ymin.sum = exp(rhssum)) %>%
  dplyr::select(adults,kids,ymin.adeq,ymin.fc,ymin.sum)
...
# Subjective poverty line estimates from minimum income questions
...
```{r order-spl-table, dependson="order-spl; some-functions", results="asis", size = "footnotesize"}

kable(zed, format = "latex", align='rrrrr',
      row.names = F,
      booktabs=TRUE, escape=FALSE, longtable=TRUE,
      linesep="",
      caption="Estimate of subjective poverty lines based on minimum household income, underpinned by linear regression model",
      col.names=c("Adults","Kids","Food","and Clothing","and Housing")) %>%
  add_header_above(c(" "=2, "Consumption Adequcy in"=3,)) %>%
  kable_styling(latex_options = c("striped", "hold_position", position="center")) %>%
  footnote(general = "Estimated poverty lines by household type (adults and kids), netting out affects related to location, and other household head controls. The columns are inclusive, such that the first is for food adequacy only, while the second is food and clothing, and the third is food, clothing and housing.", escape=FALSE, general_title="",threeparttable=TRUE) 
```
