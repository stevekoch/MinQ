...
spl.tab <- cbind(spl.lin.yall.table, spl.quad.yall.table[,3], zed[,3:5])

kable(zed, format = "latex", align='rrrrrrr',
      row.names = F,
      booktabs=TRUE, escape=FALSE, longtable=TRUE,
      linesep="",
      caption="Estimate of subjective poverty lines based on minimum household income, underpinned by linear regression model",
      col.names=c("Adults","Kids", "Linear", "Qadratic","Food","and Clothing","and Housing")) %>%
  add_header_above(c(" "=2, "Minumum Income"=2, "Consumption Adequcy in"=3)) %>%
  kable_styling(latex_options = c("striped", "hold_position", position="center")) %>%
