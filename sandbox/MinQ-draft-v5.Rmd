...
We apply standard methods, assuming a parametric relationship \eqref{eq:nprelation} between minimum income $(\underline{Y})$, actual income $(Y)$, household demographic characteristics $(D)$ and other characteristics related to household decisions, such as location and marital status, $(Z)$. Equation \eqref{eq:nprelation} represents the general structure, which assumes no functional form. For sensitivity, we non-parametrically estimate \eqref{eq:nprelation}. We discuss the error term, below.
\begin{equation}
\label{eq:nprelation}
\ln \underline{Y} = f(Y,D,Z)
\end{equation}
We tighten the functional form assumption, first, assuming linearity of income. Treating the number of adults and children as continuous variables yields expression \eqref{eq:continuous}.
\begin{equation}
\label{eq:continuous}
\ln \underline{Y} = \alpha + \beta \ln Y + \phi A_i + \theta K_i + \sum_j \zeta_j Z_j 
\end{equation} 
Further, we follow @koch2022 and @koch2023, creating a dummy variable representation of household structure, as in \eqref{eq:linindex}. Within the expression, we define $a_{ia} = \mathbf{1}(A_i=a)$, with the kids dummy variables similarly defined.
\begin{equation}
\label{eq:linindex}
...
\begin{equation}
\label{eq:quad}
\ln \underline{Y} = \alpha + \beta \ln Y + \gamma \left( \ln Y \right)^2 + \sum_j \delta_j D_j + \sum_j \zeta_j Z_j 
\end{equation}
...
# Sensitivity: nonparametric regression
...
# Age gradient for minimimum income
