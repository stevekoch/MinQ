...
poor.data <- left_join(main.df, spl.lin.yall,
  ...
  mutate(hhsize = as.numeric(ao) + as.numeric(ko),
         ...
         x = exp(lnx)) %>%
  group_by(ao,ko) %>%
  summarise(lin.subj.poor = round(100*sum(y < spl.linear)/n(),2),
...
kable(poor.data, format = "latex", align='rrrrrr',
      row.names = F,
      booktabs=TRUE, escape=FALSE, longtable=TRUE,
      linesep="",
      caption="Precentage of poor households by household structure type based on subjective and objective poverty lines",
      col.names=c("Adults","Kids","Linear","Quadratic","Lower","Upper")) %>%
  add_header_above(c(" "=2, "Subjective Measures"=2, "Objective Measures" = 2)) %>%
  kable_styling(latex_options = c("striped", "hold_position", position="center")) %>%
  footnote(general = "Estimated poverty percentages based on subjective poverty lines underscored by linear and quadratic estimates of minimum income levels compared to lower (ZAR 647 per capita in 2014) and upper bound poverty (ZAR 992 per capita) lines from national treasury. Percentages not weighted to the population.", escape=FALSE, general_title="",threeparttable=TRUE) 
...
What would happen if we only looked at households with reported minimums near their actual incomes?

```{r lcs-near, dependson="lcs"}

subset.data <- main.df %>%
  filter(abs(lny - lnminq) <= 0.5)

```


```{r lineary-spl-near, dependson="lcs-near; lcs-sub"}

base.data <- subset.data %>%
  mutate(ao = as.numeric(ao),
         ko = as.numeric(ko))
...
```{r linearx-spl-near, dependson="lcs-near; lcs-sub"}
...
```{r quady-spl-near, dependson="lcs-near; lcs-sub"}
...
```{r quadx-spl-near, dependson="lcs-near; lcs-sub"}
...
```{r poor-data-near, dependson="lcs;lineary-spl-neat; quady-spl-near", results="asis"}
...
# Subjective poverty line estimates 
...
spl0 <- estab(spl.lin.yall.table) %>% rename(yall=results)
spl1 <- estab(spl.quad.yall.table) %>% rename(yquad=results)
...
```{r spl-table-near, dependson="lineary-spl-near; linearx-spl-near; quadx-spl-near; quady-spl-near; some-functions", results="asis", size = "footnotesize"}
...
kable(spl.tab, format = "latex", align='rrrr',
      ...
      booktabs=TRUE, escape=FALSE, longtable=TRUE,
      linesep="",
      caption="Estimate of subjective poverty lines based on minimum household income, underpinned by linear regression model - using only households with income that is near the minimum",
      col.names=c("Adults","Kids","Linear","Quadratic","Linear","Quadratic")) %>%
  add_header_above(c(" "=2, "Household Income"=2, "Household Expenditure" = 2)) %>%
  kable_styling(latex_options = c("striped", "hold_position", position="center")) %>%
  footnote(general = "Estimated poverty lines by household type (adults and kids), netting out affects related to location, and other household head controls. Bootstrapped standard errors (399 replications) are reported in parenthesis. Estimates underpinned by linear parametric models of (log) minimum income income required by the household against a range of controls including reported household (log) income or expenditure (and the matching squared term, in the case of quadratic), household structure and location dummies.", escape=FALSE, general_title="",threeparttable=TRUE) 
